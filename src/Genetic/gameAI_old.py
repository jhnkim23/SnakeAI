...
class Snake:
    def __init__(self, display=True):
        ...
        self.food = (random.randint(0, self.cols-1),
        ...
        if self.display:
            ...
            self.screen = pygame.display.set_mode(
                ((self.block_size + 2) * self.cols, (self.block_size + 2) * self.rows))
    ...
    def draw_rect(self, color, row, col):
        pygame.draw.rect(self.screen, color, (row*(self.block_size+2)+1,
                                              col*(self.block_size+2)+1, self.block_size, self.block_size))

    def step(self, action):
        if self.display:
            # pygame.time.delay(100)
            pygame.display.set_caption(
                "Snake (Score: " + str(self.score) + ")")
        ...
        initial_dist_to_food = math.sqrt(
            (self.food[0] - self.snake[0][0])**2 + (self.food[1] - self.snake[0][1])**2)
